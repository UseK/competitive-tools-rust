    ...
    pub fn movable_positions(&self, x: usize, y:usize) -> Vec<(usize, usize)> {
        ...
        for dy in -1..=1 {
            for dx in -1..=1 {
                ...
                if (x == 0 && dx == -1) || (y == 0 &&  dy == -1) { continue };
                let new_x = (x as isize + dx) as usize;
                let new_y = (y as isize + dy) as usize;
                if (self.columns <= new_x) || (self.rows <= new_y) {continue};
...
mod tests_maze {
    ...
    fn test_poj2386() {
        ...
        let field: Vec<Vec<bool>> = input_lines.into_iter().map(|line| {
            line.chars().map(|c| c == 'W').collect()
        }).collect();
        let mut maze: Maze<bool> = Maze {rows: n, columns: m, field} ;
        let mut ans = 0;
        for x in 0..m {
            for y in 0..n {
                if maze.field[y][x] {
